...
const Permissions = () => {
    ...
    let checkpermissions = () => {
        ...
        let userPermissions = token.filter(permission => permission.includes("permissions"));
    ...
    const handleDeleteConfirmation = () => {
        ...
        axios.delete(configData.API_SERVER + `roles/${userToDeleteId}`, {
            headers: {
    ...
    const handleUpdateUser = () => {
        ...
        let tokenObj = JSON.parse(Lantern)
        let token = tokenObj.token
        axios.put(configData.API_SERVER + `permissions/${id}`, data, {
            headers: {
                'Authorization': JSON.parse(token)
            }
        })
    ...
    return (
        <MainCard title="Permission de rôle">
            <Snackbar
                anchorOrigin={{
                    vertical: 'top',
                    horizontal: 'right',
            ...
            </Dialog>
            {
                forbidden ? "Vous n'êtes pas autorisé à continuer" :
                    <>
                        {usersPermission.includes('permissions_add') &&
                            <Grid container justifyContent="flex-end" item xs={12} onClick={() => history.push(`/ajoute-permission`)}>
                                ...
                                <Table>
                                    <TableHead>
                                        <TableRow>
                                            ...
                                            {(usersPermission.includes('permissions_delete') || usersPermission.includes('permissions_update')) &&
                                    ...
                                    <TableBody>
                                        {filteredUsers.slice((page - 1) * rowsPerPage, page * rowsPerPage).map((row, i) => (
                                            <TableRow key={row.id}>
                                                ...
                                                <TableCell>
                                                    <>
                                                        {usersPermission.includes('permissions_update') &&
                                                            <IconButton
                                                                style={{ color: "#2073c4" }}
                                                                aria-label="edit"
                                                                onClick={() => history.push({
                                                                    pathname: `/edit-permission-rôle/${row.id}`,
                                                                    state: { role: row.role_name }
                                                                })}
                                                            >
                                                                <EditIcon />
                                                            </IconButton>}
                                                        {usersPermission.includes('permissions_delete') &&
